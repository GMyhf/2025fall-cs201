...
Updated 1821 GMT+8 Nov 17, 2025
...
**广度优先搜索（BFS）** 是一种图遍历算法，它会先访问图中当前深度的所有顶点，然后再进入下一层深度的顶点。该算法从指定的起始顶点开始，首先访问其所有相邻顶点，之后再依次访问这些相邻顶点的下一层邻居。**BFS** 常用于路径查找、连通分量检测以及图中的最短路径问题等算法中。
...
- **最短路径查找**：BFS 可用于在无权图中查找两个节点之间的最短路径。通过在遍历过程中记录每个节点的父节点，即可重构出最短路径。
- **环检测**：BFS 可用于检测图中是否存在环。如果在遍历过程中某个节点被访问了两次，则表明图中存在环。
- **连通分量识别**：BFS 可用于识别图中的连通分量。每个连通分量是一组彼此可达的节点集合。
...
在图的深度优先搜索（DFS）中，我们会逐个遍历所有相邻的顶点。当我们<mark>遍历到一个相邻顶点时，我们会首先彻底完成从该相邻顶点出发可以到达的所有顶点的遍历</mark>。这类似于树的遍历过程，在树中我们先完全遍历左子树，然后再遍历右子树。主要的不同之处在于，**与树不同的是，图中可能包含环路（一个顶点可能会被访问多次）**。为了避免重复处理同一个顶点，我们使用一个布尔型的 `visited` 数组来记录已经访问过的顶点。
...
从`start_vertex`开始，`dfs_visit`方法尽可能深地探索所有相邻的白色顶点。如果仔细观察`dfs_visit`的代码并且将其与bfs比较，应该注意到二者几乎一样，除了内部for循环的最后一行，`dfs_visit`通过递归地调用自己来继续进行下一层的搜索，bfs则将顶点添加到队列中，以供后续搜索。有趣的是，bfs使用队列，`dfs_visit`则<mark>使用栈。我们没有在代码中看到栈，但是它其实隐式地存在于`dfs_visit`的递归调用中</mark>。
...
*深度优先搜索树（林）的性质
 ...
 1. **顶点大小的定义（基于结束时间）：** 对于图 G 中的任意两个顶点 u 和 v，当且仅当顶点 u 的结束时间（fin）小于顶点 v 的结束时间（fin）时，称顶点 u 小于顶点 v，即符号化表示为：$∀ u, v \in G; u < v \equiv u.fin < v.fin$。
    ...
    > - 所有顶点按 *f* 值从大到小排列，就是经典的 **“完成时间逆序”**，在拓扑排序、强连通分量（SCC）等算法中有关键作用。
 ...
 2. **子树大小的定义（基于所有节点的时间顺序）**： 对于深度优先搜索树（林）中的任意两棵不相交的子树 t1 和 t2，当且仅当 t1 中的任意节点的结束时间都早于 t2 中的任意节点的开始时间时，称 t1 小于 t2，即符号化表示为：$t1 < t2 \equiv ∀u \in t1, v \in t2; u < v$。
...
> ### 3. **节点间的互斥关系（无跨子树边）**
>
> > 若 ( t_1 < t_2 )，且 ( u \in t_1, v \in t_2 )，则 **不存在边 ( u \rightarrow v )**。
>
> - 这个结论**仅在特定条件下成立**，需谨慎理解。
> - 实际上，在**无向图**的DFS中，如果 ( t_1 ) 和 ( t_2 ) 是同一连通分量中的两棵子树（即属于同一DFS树），那么它们之间**不可能完全无连接**，否则不会在同一连通分量。
> - 但若 ( t_1 ) 和 ( t_2 ) 属于**不同的DFS树**（即来自不同连通分量），则确实**不存在任何边**（否则它们会被合并成一棵树）。
> - 在**有向图**中，即使 ( t_1 < t_2 )，也可能存在从 ( t_1 ) 到 ( t_2 ) 的边（称为**前向边**或**交叉边**），所以该结论**并非普遍成立**。
>
> ✅ **更准确的理解**： 
>
> > 如果在DFS过程中，**( t_2 ) 是在 ( t_1 ) 完全结束后才开始遍历的**（即 ( t_1 < t_2 )），那么**不可能存在从 ( t_1 ) 中某点指向 ( t_2 ) 中某点的树边或后向边**。
> > 但在有向图中，仍可能存在**交叉边**（cross edge）( u \to v )，此时 ( u.f > v.f )，与 ( t_1 < t_2 ) 矛盾——因此，若已知 ( t_1 < t_2 )（即所有 ( u.f < v.f )），则**确实不可能存在 ( u \to v ) 的边**，否则DFS会在访问 ( u ) 时就进入 ( v )，导致 ( v ) 被包含在 ( t_1 ) 中。
>
> 👉 所以，在**正确构建的DFS森林**前提下，该性质成立。
>
> ------
>
> 4. **同一棵树中顶点的相对关系**
>
> > 若 ( u < v )（即 ( u.f < v.f )）且 ( u, v ) 在同一棵DFS树中，则只有两种可能：
>
> 情况一：**( v ) 是 ( u ) 的祖先**
>
> - 此时根据 Corollary 22.8，应有 ( v.d < u.d < u.f < v.f )。
> - 但这里假设 ( u.f < v.f )，所以 **这种情况不可能发生**！
> - ❗ 注意：**如果 ( u.f < v.f )，则 ( v ) 不可能是 ( u ) 的祖先**，因为祖先的结束时间一定更晚。
>
> 情况二：**( u ) 和 ( v ) 有共同祖先 ( t )，分别属于 ( t ) 的不同子树 ( t_1, t_2 )，且 ( t_1 < t_2 )**
>
> - 这才是正确的情形。
> - 例如：节点 ( t ) 先访问子树 ( t_1 )（含 ( u )），完成后才访问子树 ( t_2 )（含 ( v )）。
> - 因此：
>   ( t.d < u.d < u.f < v.d < v.f < t.f )
>   ⇒ ( u.f < v.f ) ⇒ ( u < v )
> - 这符合DFS的“深度优先、回溯后继续”的行为。
>
> ✅ **修正理解**：
> 原文“顶点 v 是顶点 u 的祖先”这一说法**与 ( u.f < v.f ) 矛盾**，应为笔误或表述不清。
> 正确结论是：
>
> > 若 ( u ) 和 ( v ) 在同一DFS树中，且 ( u.f < v.f )，则要么：
> >
> > - ( u ) 是 ( v ) 的祖先（此时 ( u.d < v.d < v.f < u.f )，即 ( u.f > v.f )，矛盾）， 
> > - **或者** ( u ) 和 ( v ) 分属某个共同祖先下的不同子树，且 ( u ) 所在子树先被遍历（即 ( u.f < v.d )）。
>
> 因此，**唯一可能的情况是第二种**。第一种情况在 ( u.f < v.f ) 下不可能成立。
>
> ------
>
> **总结：DFS时间戳的核心性质**
...
> | 性质             | 说明                                                         |
> | ---------------- | ------------------------------------------------------------ |
> | **区间嵌套**     | 后代的时间区间严格包含于祖先区间：( u.d < v.d < v.f < u.f )  |
> | **子树分离**     | 不同子树的时间区间不重叠，且按遍历顺序先后排列               |
> | **边的分类限制** | 时间戳关系决定了边的类型（树边、后向边、前向边、交叉边）     |
> | **完成时间序**   | 所有顶点按 ( f ) 降序排列，可用于拓扑排序（DAG）或Kosaraju算法（SCC） |
>
> 这些性质是理解DFS在图算法中强大作用的基础。
